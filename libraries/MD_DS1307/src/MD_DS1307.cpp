...
// Perform a control action on item, using the value
{
   ...
   switch (item)
   {
      case DS1307_CLOCK_HALT:
         ...
         switch (value)
         {
            case DS1307_ON:  cmd = CTL_CH; 
               break;
            case DS1307_OFF: cmd = 0; 
               break;
            default: 
               return; // wrong - just go back
         }
         break;
      ...
      case DS1307_SQW_TYPE_ON:
         ...
         switch (value)
         {
            case DS1307_SQW_1HZ:  cmd = 0x00;	
               break;
            case DS1307_SQW_4KHZ: cmd = 0x01;	
               break;
            case DS1307_SQW_8KHZ: cmd = 0x02;	
               break;
            case DS1307_SQW_32KHZ:cmd = 0x03;	
               break;
            default: 
               return; // wrong - just go back
         }
      ...
      case DS1307_SQW_TYPE_OFF:
         ...
         switch (value)
         {
            case DS1307_SQW_LOW:  cmd = 0;	
               break;
            case DS1307_SQW_HIGH: cmd = CTL_OUT;	
               break;
            default: 
               return; // wrong - just go back
         }
...
// Return DS1307_ERROR otherwise.
{
   ...
   {
      case DS1307_CLOCK_HALT:		
         return(bufRTC[ADDR_CTL_CH] & CTL_CH ? DS1307_ON : DS1307_OFF);
      case DS1307_SQW_RUN:		
         return(bufRTC[ADDR_CTL_SQWE] & CTL_SQWE ? DS1307_ON : DS1307_OFF);
      case DS1307_SQW_TYPE_ON:
         switch (bufRTC[ADDR_CTL_RS] & CTL_RS)
         {
            case 0: 
               return(DS1307_SQW_1HZ);
            case 1: 
               return(DS1307_SQW_4KHZ);
            case 2: 
               return(DS1307_SQW_8KHZ);
            case 3: 
               return(DS1307_SQW_32KHZ);
         }
         break;	// in case of error
      case DS1307_SQW_TYPE_OFF:	
         return(bufRTC[ADDR_CTL_OUT] & CTL_OUT ? DS1307_SQW_HIGH : DS1307_SQW_LOW);
      case DS1307_12H:			
         return(bufRTC[ADDR_CTL_12H] & CTL_12H ? DS1307_ON : DS1307_OFF);
   }
